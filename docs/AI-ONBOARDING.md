...
**`.gitignore` y `.gitattributes` (no en el remoto):** por decisión del mantenedor **no** se versionan en GitHub (menos superficie visible y sin listar rutas locales en la raíz del repo). Mantén una copia local de **`.gitignore`** en la raíz (el archivo se ignora a sí mismo para no poder añadirlo sin `-f`) o usa **`.git/info/exclude`** con patrones equivalentes. Tras clonar, si no tienes esas reglas, crea al menos `bin/`, `obj/`, `.vs/` en un `.gitignore` local. **`InstalarFUEngine.exe`** no va en el historial de Git (tamaño); genéralo con **`installer/build-installer.ps1`** o publica el binario en **Releases** / otro canal.
