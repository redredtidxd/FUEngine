...
- **Dónde:** menú **Ayuda** (manual rápido/completo, API Lua, logs, GitHub), botón **Ayuda / Guía del motor** en la pantalla de inicio. Host: [`DocumentationHostControl.xaml`](../FUEngine/Controls/DocumentationHostControl.xaml) — pestañas **Manual del motor** · **Lua** (`lua-kw-*`, `lua-guide-*`, `lua-reference-intro`) · **Ejemplos** (`script-ex-*`). El **Manual** muestra prefijos de categoría en el subtítulo (p. ej. **Editor ·**, **Lua (runtime) ·**, **Mapa ·**) para ordenar temas. **«Completa»** abre el manual largo en `crear-juego` (`FullManualStartTopicId`).
...
`DocumentationView` compone el detalle en un **`RichTextBox` solo lectura** (`FlowDocument`) para que el usuario pueda **seleccionar y copiar** título, secciones y párrafos; el snippet Lua sigue en **AvalonEdit** (`TextEditor` read-only). Orden: Para qué → Por qué importa → En FUEngine → Contenido → Puntos clave → código si existe. `PageWidth` se deriva del **`ScrollViewer`** padre (`ViewportWidth`) además de `ActualWidth` del `RichTextBox`. Layout: ítem 1\* / contenido 6\* (máx. ~260 px lista), **`GridSplitter`** entre columnas; overlay Hub/editor **ocupa casi toda la ventana** (`Stretch` + márgenes ~24 px, `MaxWidth`/`MaxHeight` 2000 para pantallas grandes).
...
- **Filtros y lista:** `DocumentationView.ApplyFilter` asigna `CollectionViewSource` + `GroupDescription` por categoría (`DocumentationTopicListEntry` / `DocumentationTopicListGrouping.cs`); desuscribe `SelectionChanged` al refrescar `ItemsSource`; scroll del detalle en `ApplicationIdle`. Overlays capturan clics en el borde interior (`e.Handled = true`).
...
| FUEngine.Runtime | Core, Graphics.Vulkan, NLua (NuGet) | GameLoop, LuaScriptRuntime, Camera, SceneManager, WorldApi/InputApi/…, generación procedural de tiles (Lua), debug draw |
...
**Grafos de dependencias (sin ciclos):** Core → nada. Editor → Core. Service → Core. Graphics.Vulkan → Core (**único** ensamblado con paquetes Silk.NET). **Runtime → Core + Graphics.Vulkan** + NLua (`GameLoop.Start()` crea `VulkanGraphicsDevice.Create()` si no hay dispositivo). La app WPF → Core + Editor + Runtime + Service (no referencia Vulkan directamente; usa el runtime). **Play embebido** en el IDE: preview con WPF (`GameViewportCanvas` / `GameViewportRenderer`), sin surface Vulkan dentro del panel.
...
- Al abrir en el editor, **`ProjectFormatOpenHelper.TryPromptAndLoad`**: si el formato es menor que el actual, diálogo **Sí** (migración + guardar; **antes** intenta **`Project.FUE` → `Project.FUE.bak`**; si la copia falla, aviso y puede guardarse igual), **No** (abrir sin tocar el .FUE; `ProjectInfo.FormatMigrationDeclinedAtOpen`; aviso en consola — **no** hay modo solo lectura global), **Cancelar**. Pasos en **`ProjectFormatMigration.MigrateToVersion`** (Editor; extensible a transformaciones, no solo campos nuevos). El reproductor mínimo aplica migración solo en memoria (`ApplySilentInMemory`).
...
- **§24** resume stack y archivos; el **Manual completo** en la app reparte el mismo contenido en temas por **`DocumentationTopic.Id`**, por ejemplo: `quick-start`, `crear-juego` (secciones numeradas 1–9 + limitaciones + `LuaExampleCode` en alineación con `WorldApi.setTile`), `arquitectura`, `version-motor-compatibilidad`, `pestañas`, `vulkan-ventana-juego`, `naudio-audio-proyecto`, `avalonedit-scripts-ide`, `pestanas-editor-catalogo`, `componentes-catalogo`, `manual-varios-temas`, `jerarquia-gameobject-lua`, `bootstrap-script`, `tiledata-collision-flags`, `deshacer-rehacer-editor`, `pantalla-inicio-hub`, `asistente-nuevo-proyecto-jerarquia` (Explorador + asistente), `idioma-tema-editor`, `medidores-gameplay-flags`, `lua-completion-catalogo`. **No** existe tema in-app «compilar desde fuente»: build del motor se remite a **README.md**.
...
| `IScriptRegistryWriter` | Escritura de `scripts.json` (`ScriptRegistryProjectWriter.TryRegisterLuaFile`, etc. — p. ej. **Explorador → Nuevo Script Lua**; no documentar como paso manual editar el JSON para ese flujo). |
