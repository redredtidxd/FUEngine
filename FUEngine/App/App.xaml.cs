...
public partial class App : System.Windows.Application
{
    ...
    private void App_OnStartup(object sender, StartupEventArgs e)
    {
        ...
        try
        {
            ...
            splash.RunThenClose(() =>
            {
                if (TryOpenEditorFromCommandLine(e.Args))
                    return;
    ...
    /// <summary>Abre el editor con un proyecto si la línea de comandos trae una ruta a Project.FUE, .json o carpeta del proyecto.</summary>
    private bool TryOpenEditorFromCommandLine(string[]? args)
    {
        if (args == null || args.Length == 0) return false;
        var first = args[0].Trim().Trim('"');
        if (string.IsNullOrEmpty(first) || first.StartsWith('-')) return false;
        ...
        try
        {
            if (File.Exists(first))
            {
                ...
                if (string.Equals(Path.GetExtension(first), ".FUE", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(Path.GetExtension(first), ".fueproj", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(name, "proyecto.json", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(name, "Project.json", StringComparison.OrdinalIgnoreCase))
                    projectPath = Path.GetFullPath(first);
            }
            else if (Directory.Exists(first))
            {
                var dir = Path.GetFullPath(first);
                var fue = Path.Combine(dir, "Project.FUE");
                var legacy = Path.Combine(dir, "proyecto.json");
                var pj = Path.Combine(dir, "Project.json");
                if (File.Exists(fue)) projectPath = fue;
                else if (File.Exists(legacy)) projectPath = legacy;
                else if (File.Exists(pj)) projectPath = pj;
            }
        }
        catch
        {
        ...
        try
        {
            if (!ProjectFormatOpenHelper.TryPromptAndLoad(projectPath, null, out var project, out var loadErr))
            {
                if (!string.IsNullOrEmpty(loadErr))
                    EditorLog.Warning($"Argumento de proyecto: {loadErr}");
                return false;
            }

            if (project == null) return false;
            StartupService.AddRecentProject(projectPath, project.Nombre, project.Descripcion, EngineVersion.Current);
            OpenEditor(project);
            return true;
        }
        catch (Exception ex)
        {
            EditorLog.Warning($"Argumento de proyecto: {ex.Message}");
            return false;
        }
    }
