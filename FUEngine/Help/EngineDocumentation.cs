        ...
        var topics = new List<DocumentationTopic>
    {
        ...
        new(
            ...
            paragraphs: new[]
            {
                ...
                "**6. Scripts y `scripts.json`** — En **Play** solo corren `.lua` que figuran en **`scripts.json`**. **En la práctica no editas ese archivo a mano:** crear o importar scripts desde el **Explorador** (y ejemplos desde Ayuda) **sincroniza el registro**. Si arrastras o eliges un `.lua` que aún **no** está registrado, el Inspector **lo registra solo** (`scripts.json`). **Jerarquía:** puedes soltar un `.lua` sobre un **objeto** (adjunta script) o sobre la **capa/carpeta de objetos** (crea objeto y adjunta).",
    ...
    if input.isKeyDown(Key.Space) then
        ...
        new(
            ...
            paragraphs: new[]
            {
                ...
                "Jerarquía — trigger: Nuevo Trigger Zone, **Nuevo Click Trigger** (instancia con área clicable en el viewport Play; objetos.json, no triggerZones.json), Duplicar, Eliminar, Renombrar, Propiedades.",
        ...
        new(
            ...
            paragraphs: new[]
            {
                ...
                "Un objeto: ObjectInspector — cabecera (nombre, ID abreviado, activo en Play), bloque Identidad (GUID solo lectura, regenerar, tipo, etiquetas, capa Z, visible), Transform, luz puntual opcional, expander «Sprite, física y gameplay (Play)»: tinte/flip/sort, clip de animación por defecto, forma de colisión Box/Circle y tamaños, rigidbody, CameraTarget, ProximitySensor, Health, AudioSource, **clic/tap en mundo (ClickInteractable: forma, offsets, hover PC, filtro ratón/toque, distancia máx. al jugador, scripts opcionales)**, ParticleEmitter (persistidos en objetos.json). Interacción y scripts.",
                "Scripts: **varios** por objeto (lista ordenada = orden en Play). Combo **Agregar**, botón **+** (elegir `.lua`), **arrastrar** `.lua` desde el sistema o desde el **Explorador**; registro automático en `scripts.json` si hace falta. **Jerarquía:** soltar `.lua` sobre objeto o sobre capa/carpeta de objetos. Aviso suave si el archivo no parece definir onStart/onUpdate (no bloquea). «Añadir componente…»: sprite, colisión, rigidbody, luz, animación, audio, proximidad, salud, cámara, partículas, **clic en mundo**. Arrastrar un objeto desde la jerarquía sobre un campo tipo referencia rellena el InstanceId.",
                "Propiedades del script: `-- @prop`, variables globales en la raíz del `.lua` (sin `local`) y comentario `-- [Editable]` al final de línea como marca explícita; tipos incl. object (InstanceId). Con Play activo, sincronización hot y edición al perder foco. Expander «Variables de script (@prop, -- [Editable], globales)» (ver «Conectar scripts con el motor»).",
        ...
        new(
            ...
            paragraphs: new[]
            {
                ...
                "ClickInteractableComponent: área **Box**, **Circle** o **PixelPerfect** (alpha del sprite de la definición); **ClickZPriority** desempata encima de otros solapes antes del orden de render; **RequireLineOfSight** usa rayo DDA en tiles con colisión; **InteractEnabled** en datos corta hit sin quitar el componente; **OnPressScaleMul** encoge el transform al pulsar. Transform del objeto (posición, escala, rotación) define el marco del sprite y el área geométrica. Lua: **clickInteract.bind(self, \"down\"|\"up\"|\"pointerclick\"|\"click\"|\"enter\"|\"exit\", …)** (también nombres *pointerdown* / *onPointerClick* normalizados); hooks **onWorldPointerDown**, **onWorldPointerUp**, **onWorldPointerClick**, **onWorldClick**, **onWorldPointerEnter**, **onWorldPointerExit**. **HoverTintHex** queda persistido para futuro tinte en visor. En móvil sin puntero, hover no aplica."
            },
            bullets: new[]
            {
        ...
        new(
            ...
            bullets: new[]
            {
                "JSON: propiedades camelCase (spriteColorTintHex, rigidbodyEnabled, proximitySensorEnabled, clickInteractableEnabled, clickInteractableShape, …).",
        ...
        new(
            ...
            paragraphs: new[]
            {
                ...
                "self (SelfProxy): id, name, tag, tags[], hasTag, x, y, rotation, scale, visible, active, renderOrder; properties — proxy tipo tabla (p. ej. self.properties[\"x\"], tintR/tintG/tintB o color.r/g/b, renderOrder, visible) para reflejar en runtime campos habituales del Inspector sin sustituir a x/y; el mismo proxy existe en otros objetos: world.findObject(\"Nombre\"):properties[\"clave\"] lee propiedades de Inspector de ese objeto; move, rotate, destroy; callScript(ruta.lua, \"funcion\", …) — llama una función global definida en la instancia de ese .lua en **este** GameObject (mensajes de error en consola Play con prefijo [callScript]); find, findInHierarchy, getParent, getChildren, setParent; getComponent, removeComponent; setSpriteTexture, addSpriteFrame, clearSpriteFrames, spriteFrame, setSpriteAnimationFps, setSpriteSortOffset, setSpriteDisplaySize, setSpriteTint(r,g,b); playAnimation(clipId) / stopAnimation — aplican clips de animaciones.json al SpriteComponent en Play; instantiate(prefab, x, y, rot?, variant?).",
                "world: findObject / getObjectByName, findObjectByInstanceId(id), findByTag / getObjectByTag / getObjects / getAllObjects, findByPath, findNearestByTag, spawn/instantiate, destroy, setPosition, getPlayer, getTile(x,y,layerName), setTile(x,y,layerName,catalogId), raycast(origen, dir, maxDist, ignore?), raycastTiles, raycastCombined. Mensajería entre scripts: local tablero = world.findObject(\"Tablero\"); tablero:callScript(\"Scripts/TicTacToe.lua\", \"marcarCasilla\", x, y) — requiere que Tablero tenga ese .lua asignado.",
                ...
                "native: puente opcional a **FUECoreNative.dll** (C/C++ junto al ejecutable). **Lua nunca llama a la DLL directamente:** solo C# (`NativeBridge`) y la tabla `native` (p. ej. `native.isAvailable()`, `native.fastMathSum(a,b)`, `native.versionString()`). Si falta la DLL, el motor sigue en Play y la consola muestra un aviso (categoría Native); las funciones devuelven valores seguros (0 o cadena vacía). Cadenas desde la nativa se leen con marshalling ANSI en el puente.",
        ...
        new(
            ...
            paragraphs: new[]
            {
                "findByTag y getObjects devuelven proxies o listas según API; revisa la firma en autocompletado. Cada proxy tiene **properties** (Inspector) y **callScript(ruta, función, …)** para hablar con otro script del mismo objeto.",
        ...
        new(
            ...
            paragraphs: new[]
            {
                ...
                "**Flujo al abrir (actualización segura):** si el .FUE trae **projectFormatVersion** menor que la del motor actual, aparece un diálogo. **Sí** — se aplican los pasos de **ProjectFormatMigration** en **cadena** (v1→v2→v3 como pasos sucesivos **Migrate_N_To_N+1** en código; **no** se salta de v1 a v3 sin ejecutar el intermedio). Son cambios **aditivos** en el manifiesto donde aplica; mapas, objetos y `.lua` en disco **no** se reescriben en esos pasos salvo que un paso futuro lo documente. Antes de sobrescribir el .FUE migrado, el editor intenta crear **`TuProyecto.FUE.bak`**. Si el **JSON del manifiesto está corrupto**, la carga **falla** y se registra en la **consola** (`IEditorLog`); **no** se sobrescribe el archivo original. **No** — el .FUE en disco **no** se toca; abres con formato viejo. **Cancelar** — no se abre el proyecto. Tras **No**, el editor **no** bloquea guardar globalmente: evita mezclar formato viejo y datos nuevos sin criterio.",
            ...
            bullets: new[]
            {
                ...
                "**Cadena:** solo pasos consecutivos en `ProjectFormatMigration` (prohibido saltar versiones sin métodos intermedios).",
                "**JSON corrupto:** error en consola y aborto de carga del manifiesto (no se pisa el archivo por ese fallo).",
