...
**Git (una vez por clon):** las reglas de omisión están en el archivo **`gitignore`** de la raíz (sin punto en el nombre). Actívalas con `git config core.excludesfile ./gitignore` o con **`powershell -ExecutionPolicy Bypass -File installer\setup-git-local.ps1`**. Los archivos **`.gitignore`** y **`.gitattributes`** no se suben al remoto; Git LFS para el instalador usa un **`.gitattributes` local** creado por ese script (queda ignorado y no se commitea).
...
Quien **construye** el paquete solo necesita **SDK 8**. El código del instalador está en el repo (`FUEngine.Installer\` + `installer\zip_payload.ps1`). **`InstalarFUEngine.exe`** en la **raíz** se sube con **Git LFS** (tamaño >100 MB); antes de **`git add`**, ejecuta **`installer\setup-git-local.ps1`** (o `git lfs track "InstalarFUEngine.exe"` con LFS instalado). **`FUEngine.Installer\publish\`** se omite vía el archivo **`gitignore`**. Tras regenerar el instalador: **`git add InstalarFUEngine.exe`**, commit, push. Para clonar y obtener el `.exe` real: **[Git LFS](https://git-lfs.com)** y **`git lfs pull`** si hiciera falta.
...
La carpeta **`tools\`** está listada en **`gitignore`** (sin punto): no forma parte del repositorio. Puedes crear ahí tus propios `.bat` o scripts (por ejemplo un `publicar.bat` que llame a `dotnet publish` con una carpeta con fecha, o accesos rápidos personales). Cada colaborador mantiene la suya en local.
